% compute the negative log-likelihood given a set of parameters and data
% params: a vector of parameters
% data: a data table consisting of a participants trials
% fitOpts: structure noting which parameters should be fit, and the default values of those that are not
function [negLLE, fitData] = getLLE_nInit_uUtil(params, data, fitOpts)
    ...
    uL          = fitData.transParams(5);
    uU          = fitData.transParams(6);
    % binary flag: use familirity gate?
    uGate       = fitData.transParams(7);
    ...
    for tI = 1 : length(data.trialID)
        ...
        if ~isnan(data.selectedStimID(tI))
            ...
            beta                    = 1;
            fitData.nVal(tI,:)      = (alpha .* beta) ./ ( (alpha+beta).^2 .* (alpha + beta + 1) ) / normTerm;
